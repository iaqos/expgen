...
class SummarizationPVP(GenerativePVP, ABC): # AAA
    def get_parts(self, example: InputExample) -> FilledPattern:
        text_a = self.shortenable(example.text_a)
        ...
        if self.pattern_id == 0:
            ...
            return  [text_a] + prefix + [self.mask], [] # moved mask after input for untrained scoring
        elif self.pattern_id in [2, 4]:
            return [text_a] + prefix + [self.mask], []
...
class CnnDailymailPVP(SummarizationPVP): # AAA
    def generative_prefix(self) -> Optional[str]:
        if self.pattern_id in [0, 1]:
            return None
        elif self.pattern_id in [2, 3]:
