...
class MLMPreprocessor(Preprocessor):
    ...
    def get_input_features(self, example: InputExample, pattern_id: int, labelled: bool, priming: bool = False,
        ...
        if self.wrapper.config.model_type == 'pegasus' or self.wrapper.config.model_type == 't5':
            # bugfix: Transformers' create_token_type_ids_from_sequences seems to ignore the final </s> token in Pegasus and T5
            token_type_ids += [0]
