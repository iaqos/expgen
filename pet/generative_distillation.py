...
def compute_output_probabilities(examples: List[GenerativeInputExample], output_dir: str, eval_config: pet.config.EvalConfig,
    ...
    for example in examples: 
        # AAA bugfix for example 132970, tentative...
        
        if not idx_to_outputs[example.idx] and int(example.idx) == 132970:
            pass
        else:#example.output_text == "Two dogs running is in both sentences."
            #idx_to_outputs[example.idx] == "Two dogs running is in both sentences."
            print(">>>>", example.idx, example.text_a, "OUTPUT: ", example.output_text, "idx_to_out: ", idx_to_outputs[example.idx])
            pass #raise ValueError(f"No outputs found for example {example.idx}")
    ...
    for example in examples: # AAA problema
        if example.idx not in idx_to_examples:
            print(">>>> DISTILLATION", example.idx, "OUTPUTTEXT: ", example.output_text)
            pass #raise ValueError(f"No outputs were found for input example {example.idx}")
