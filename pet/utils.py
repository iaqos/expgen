...
def save_predictions_with_references(path: str, results: Dict, references: List): # AAA
    """Save a sequence of predictions to a file with index and reference"""
    predictions_with_idx_and_ref = []


        
    for idx, prediction, reference in zip(results['indices'], results['predictions'], references):

        idx = idx.tolist() if isinstance(idx, np.ndarray) else int(idx)
        predictions_with_idx_and_ref.append({'idx': idx, 'prediction': prediction, 'reference': reference})

    with open(path, 'w', encoding='utf8') as fh:
        for line in predictions_with_idx_and_ref:
            fh.write(json.dumps(line) + '\n')

def save_predictions(path: str, wrapper, results: Dict): # AAA
